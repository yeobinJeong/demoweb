 ...
 */
@WebServlet("/upload/download.action")
public class DownloadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		download = dao.getUploadFileByUploadFileNo(Integer.parseInt(uploadfileno));
		...
		if (download == null || download.getUserFileName().length() == 0) {
			response.sendRedirect("detail.action");
			return;
		}
		...
		// 다운로드되는 파일의 이름을 설정 파일의 이름을 설정 (다운로드 창에 표시될 이름)
		 ...
		 String f2 = new String(download.getUserFileName().getBytes("utf-8"), "ISO-8859-1");
...
//				"attachment;filename=\"" + f2 + "\"");
		...
		response.addHeader("Content-Disposition",
				...
				"attachment;filename=\"" + f2 + "\"");
		// 파일을 응답 컨텐츠로 기록
		File f = new File(path, download.getSavedFileName());// path와 file을 이어서 경로를 구성
